...
x1=[0 for x in range(8)]
z1=[0 for x in range(8)]
...
for key in DOE_Inputs:
	T1=(DOE_Inputs[key][0])*0.75
	T1=round(T1, 2)
	ORTT=(DOE_Inputs[key][1])*20+10
	ORTT=round(ORTT, 0)
	t=(DOE_Inputs[key][2])*0.65*ORTT+0.25*ORTT
	t=round(t, 0)
	# G=(DOE_Inputs[key][5])*30+20
	# G=round(G, 0)
	# HNG=(DOE_Inputs[key][6])*30+20
	# HNG=round(HNG, 0)
	...
	fileobject.close()
	# Command line to run
	command_file = r'abaqus cae nogui=3Panel_Thermal.py'
	...
	if ((x==x1) & (z==z1)):
		...
		curve = +0.5*(z[0]+z[7])+0.16*xnew*xnew-0.04
		error = 0
