...
for key in DOE_Inputs:
	T1=(DOE_Inputs[key][0])
	T1=round(T1, 2)
	T2=(DOE_Inputs[key][1])*0.5*T1+0.5*T1
	T2=round(T2, 2)
	ORTT=(DOE_Inputs[key][2])*20+10
	ORTT=round(ORTT, 0)
	t=(DOE_Inputs[key][3])*0.65*ORTT+0.25*ORTT
	t=round(t, 0)
	...
	fileobject.close()
	# Command line to run
	command_file = r'abaqus cae nogui=4Panel_Thermal.py'
	...
	if ((x==x1) & (z==z1)):
		...
		curve = +0.5*(z[0]+z[9])+0.16*xnew*xnew-0.04
		error = 0
